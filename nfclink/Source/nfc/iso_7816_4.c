...
uint8_t ISO_7816_4_stateMachine(uint8_t * pui8PduBufferPtr, uint8_t ui8AvailableSpace)
{
	...
	case ISO_7816_4_TAG_MODE:
	    ...
	    if(g_ui16CommandResponse== CEPAS_OK){
	        unsigned char response[97] = {0x02, 0x01, 0x00, 0x00, 0x29, 0x00 , 0x07, 0xd0, 0x80, 0x08, 0x16, 0x00, 0x15, 0x97, 0x37, 0x89, 0x0a, 0x06, 0x7d, 0x32, 0x00, 0x15, 0x31, 0x58, 0x27, 0x76, 0x21, 0x99, 0x00, 0x25, 0x11, 0xff, 0x75, 0x00, 0x03, 0xe8, 0x2c, 0x89, 0xdc, 0xc9, 0x10, 0x20, 0xc3, 0x55, 0x1e, 0x00, 0xa0, 0xff, 0xff, 0x92, 0x2c, 0x99, 0xad, 0x0f, 0x54, 0x49, 0x54, 0x32, 0x37, 0x33, 0x35, 0x20, 0x02, 0x00, 0x80, 0x30, 0x2a, 0x00, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x90, 0x00};
	        for(ui8Counter=0;ui8Counter<97;ui8Counter++){
	            pui8PduBufferPtr[ui8Offset++] = response[ui8Counter];
	        }
	        break;
	    }
...
uint8_t ISO_7816_4_processReceivedRequest(uint8_t * pui8RxPayload , uint8_t ui8CmdLength)
{
	...
	//CEPAS handing here
	if(ui8CLAValue == 0x90){
	    g_ui16CommandResponse = CEPAS_OK;
	    g_ui8BytestoSend = 0x00;

	}

	else if(ui8CmdLength < 4)
	{
