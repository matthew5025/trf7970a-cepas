...
namespace TI_NFC_Tool
{
    ...
    {
        //
        // Write RW Button Click
        //
        private void button_RW_Write_Click(object sender, EventArgs e)
        {
            Byte[] ui8TxPacket;
            eRTDTypes eRTD;

            if (tabControl_RW_Tag_Write.SelectedTab == tabControl_RW_Tag_Write.TabPages["tabRWRTDText"])
            {
                ui8TxPacket = Encoding.ASCII.GetBytes(richTextBox_RW_RTD_Text.Text);
            ...
            else if (tabControl_RW_Tag_Write.SelectedTab == tabControl_RW_Tag_Write.TabPages["tabRWRTDURI"])
            {
                ui8TxPacket = Encoding.ASCII.GetBytes(richTextBox_RW_RTD_URI.Text);
            ...
            transmitRWNDEF(ui8TxPacket, eRTD);
        }

        //
        // Sends NDEF message based on which RTD Tab is open.
        //
        private void transmitRWNDEF(byte[] ui8Payload, eRTDTypes eRTDType)
        {
            Byte[] ui8TxPacket;
            ...
            switch (eRTDType)
            {
                case eRTDTypes.RTD_TEXT:
                    ...
                    SendCommand(ui8TxPacket, eNFCHostCommands.RW_WRITE_TAG, ui8TxPacket.Length);
        ...
        //
        // Helper function to keep RW Tag Content box updated
        //
        private void richTextBox_RW_Tag_Content_TextChanged(object sender, EventArgs e)
        {
            richTextBox_RW_Tag_Content.ScrollToCaret();
        }

        private void richTextBox_RW_RTD_Text_TextChanged(object sender, EventArgs e)
        {
            textBox_RW_RTD_Text_Counter.Text = "Message Length: " + richTextBox_RW_RTD_Text.TextLength + "/246";
        }

        private void button_RW_TagFormat_Click(object sender, EventArgs e)
        {
            SendCommand(null, eNFCHostCommands.RW_FORMAT_TAG, 0);
        }
